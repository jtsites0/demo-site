...
function MagneticButton({ href, children, className = '', external = false }) {
  ...
  return <a ref={ref} onMouseMove={move} onMouseLeave={reset} className={`button magnetic ${className}`} href={href} target={external ? '_blank' : undefined} rel={external ? 'noreferrer' : undefined}>{children}</a>;
}
...
function TiltCard({ children, className = '' }) {
  ...
  const move = e => {
    ...
    el.style.transform = `perspective(900px) rotateX(${(0.5 - py) * 6}deg) rotateY(${(px - 0.5) * 7}deg) translateY(-4px)`;
  };
  const reset = () => { if (ref.current) ref.current.style.transform = ''; };
  return <article ref={ref} onMouseMove={move} onMouseLeave={reset} className={`tilt-card ${className}`}>{children}</article>;
}
    ...
    <div className="site-shell">
      ...
      <header className="header">
        <div className="container nav-wrap">
          ...
          <button className="menu-toggle" aria-label="Abrir menu" onClick={() => setMenuOpen(v => !v)}><i/><i/><i/></button>
      ...
      <main>
        <section className="hero" id="inicio">
          <div className="aurora a1"/><div className="aurora a2"/><div className="grid-bg"/>
          <div className="container hero-grid">
            <div className="hero-copy reveal">
              ...
              <div className="hero-actions">
                ...
                <MagneticButton className="secondary" href="https://github.com/JTSites0" external>Ver projetos no GitHub ↗</MagneticButton>
              </div>
            ...
            <div className="hero-stage reveal">
              ...
              <TiltCard className="browser-card">
                <div className="browser-top"><span/><span/><span/><em>jtsites.dev</em></div>
                <div className="browser-screen">
                  <div className="mini-badge">JT SITES / WEB PROJECT</div>
                  <div className="mini-title"/><div className="mini-title short"/><div className="mini-btn"/>
        ...
        <section className="showcase" id="portfolio"><div className="container showcase-grid reveal"><div><span className="eyebrow">PORTFÓLIO</span><h2>Este próprio site já é parte do nosso trabalho.</h2><p>Uma vitrine viva do que a JT Sites consegue entregar em estrutura, responsividade, identidade e interação.</p><MagneticButton className="dark" href="https://github.com/JTSites0/demo-site" external>Ver código no GitHub ↗</MagneticButton></div><div className="code-window"><div className="code-top"><i/><i/><i/><span>JT_SITES.jsx</span></div><pre><code>{`const site = {\n  moderno: true,\n  responsivo: true,\n  identidade: "própria",\n  objetivo: "gerar contato"\n};`}</code></pre></div></div></section>

        <section className="section"><div className="container reveal"><div className="heading"><span className="eyebrow">PRINCÍPIOS</span><h2>Menos template. Mais identidade.</h2></div><div className="value-grid">{values.map(([n,t,d]) => <TiltCard key={n}><span className="card-num">{n}</span><h3>{t}</h3><p>{d}</p></TiltCard>)}</div></div></section>
